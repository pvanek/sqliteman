...
./svn2dist ../Sqliteman ../Sqliteman/sqliteman --no-i18n -n sqliteman -v 1.3 --log ./svn2dist.log
cp sqliteman-1.1.tar.gz /usr/src/packages/SOURCES
rpmbuild --define "builddate ${BUILDDATE}" -ba sqliteman.spec

# clean
rm -r /usr/src/packages/BUILD/sqliteman-1.3/

mv /usr/src/packages/RPMS/i586/sqliteman* .
mv /usr/src/packages/SRPMS/sqliteman* .
cp sqliteman-1.3.tar.gz sqliteman-1.3-${BUILDDATE}.tar.gz
